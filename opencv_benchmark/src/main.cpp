...
void benchmark_cpu(const std::string &device_name, int M, int N, int K,
                   int runs) {
...
void benchmark_gpu(const std::string &device_name, int M, int N, int K,
  ...
  if (!cv::ocl::haveOpenCL()) {
    std::cout
        << "[ERROR] OpenCL non è disponibile o non è abilitato in OpenCV.\n";
    return;
  }
...
#ifdef DEBUG
  std::cout << "\n=== Benchmark: " << device_name << " ===\n";
  std::cout << "Context: " << cv::ocl::useOpenCL()
            << " | Device: " << cv::ocl::Context::getDefault().device(0).name()
            << "\n";
  std::cout << "Matrix size: " << M << "x" << N << "x" << K
            << " | Runs: " << runs << "\n";
#endif
  ...
  /* Host */
  cv::Mat h_A(M, K, CV_16F);
  cv::Mat h_B(K, N, CV_16F);
  cv::RNG rng(42);
  rng.fill(h_A, cv::RNG::UNIFORM, -1.0f, 1.0f);
  rng.fill(h_B, cv::RNG::UNIFORM, -1.0f, 1.0f);
...
bool createOpenCLContext(const std::string &platform_substr) {
  ...
  if (!platformFound) {
    std::cerr << "[ERROR] Could not find platform containing: "
              << platform_substr << std::endl;
    return false;
  }
  ...
  if (context.empty()) {
    std::cerr << "[ERROR] Failed to create context with config: "
              << config.str() << std::endl;
    return false;
  }
  ...
  std::cout << "[INFO] OpenCL enabled: "
            << (cv::ocl::useOpenCL() ? "YES" : "NO") << "\n";
...
int main() {
  ...
  if (!createOpenCLContext("intel")) {
    std::cerr
        << "[WARNING] Failed to set Intel GPU context, using default device\n";
  }
#endif
