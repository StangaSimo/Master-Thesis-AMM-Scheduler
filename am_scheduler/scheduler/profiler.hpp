...
public: 
    ...
    void print_stats(const vector<BT>& bts, task* tasks, size_t num_tasks) {
        print_latency_metrics(tasks, num_tasks);
    ...
    void print_stats(const vector<BT>& bts, const vector<task*>& sub_tasks) {
        ...
        print_stats(bts, temp_tasks.data(), temp_tasks.size());
    }

private:
    void print_latency_metrics(task* tasks, size_t num_tasks) {
        if (tasks == nullptr || num_tasks == 0) {
            std::cout << "[PROFILER] No tasks to print.\n";
            return;
        }
        ...
        for (size_t i=0; i<num_tasks; i++) {
            ...
            double curr_ms = duration.count();
            
            // Ignora eventuali valori non validi (safety check)
            if (curr_ms < 0) continue;

            total_latency_sum_ms += curr_ms;
            if (curr_ms < min_latency_ms) min_latency_ms = curr_ms;
            if (curr_ms > max_latency_ms) max_latency_ms = curr_ms;
        }

        std::chrono::duration<double, std::milli> global_span = global_end - global_start;
        double average_latency_ms = (num_tasks > 0) ? total_latency_sum_ms / num_tasks : 0.0;
    ...
    void print_scheduler_metrics(const vector<BT>& bts) {
        ...
        for (BT i : bts) {
            ...
            string acc_str = get_acc_string(i); // Assicurati che questa funzione sia visibile o spostala qui
            std::cout << "[" << acc_str << "]" << "\n";
 ...
 /* prints the performance stats of the tasks (for test) */
