...
In conclusion, all the project objectives were met. The AMM Scheduler proved that an approach to hardware utilization that embraces the diversity of available accelerators rather than ignoring it, is a viable path to maximizing both throughput and energy efficiency in modern computing systems.

\section{Future Work}
While the current implementation offers a simple foundation for heterogeneous scheduling, several avenues for future research and optimization remain open to further enhance the system's capabilities:

\begin{itemize}
\item Multi-Node Scaling: The natural evolution of this project is to extend the scheduler's scope beyond a single machine. Future work could focus on adapting the coordinator to distribute workloads across a server cluster, treating remote nodes as additional accelerators. This would involve implementing network-aware dispatching logic to balance computation speed with network latency, effectively creating a distributed heterogeneous computing grid.

\item **Pre-emptive Dynamic Scheduling**: The current Dynamic logic suffers from "straggler" issues with large tasks because it cannot interrupt a running kernel. Implementing a mechanism to split large tasks at runtime or cancel and re-assign slow tasks could significantly improve the robustness of the dynamic approach against unpredictable delays.

\item **Support for Non-Matrix Workloads**: While the current architecture is highly optimized for Matrix Multiplication, the underlying abstraction layer could be generalized. Extending the scheduler to handle arbitrary compute graphs or pipeline-parallelism would make it applicable to a wider range of high-performance computing applications beyond linear algebra.
