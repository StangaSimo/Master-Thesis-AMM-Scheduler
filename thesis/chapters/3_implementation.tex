...
This static strategy addresses a specific case, handling a single matrix multiplication that is too large to fit into the memory of any individual accelerator. Instead of treating the matrix as an indivisible unit, this logic implements a Split-Compile-Merge approach, splitting the large matrix horizontally in smaller chunks and sending smaller tasks to each accelerator, enabling the sistem to process this workload in parallel, then the result are written directly into the corresponding section.  
    ...
    for (int j=0; j<bts_len; j++) {
        ...
        while (h > 0) {
            ...
            *sub_task = *single_task;
            ...
            /* for each accelerator we add more offset */
...
The core of the logic on the Listing. \ref{heterologic} relies on the \texttt{bt\_ms} array, wich tracks the load assigned to each accelerator so far (in the current batch). The load stand for the accumulated ms expected for that particular accellerator. 

The logic iterates through the batch of tasks one by one. For each task, it queries the PerformanceMap to predict how long it will take on every available accelerator, then it compute the finish time expected with the load plus the task expences, than it update the best \texttt{bt}. Finally the task then is assigned to the best accelerator that has the smallest finish time, and the current load of that accellerator is updated.  
