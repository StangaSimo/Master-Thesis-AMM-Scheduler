...
The \texttt{init\_threads} function is responsible for setting up the environment based on the available hardware:
...
\begin{lstlisting}[caption={Benchmark Accelerator Function}, language=C++] 
...
As explained in the previous section the raw data is acquired through automated benchmark routines (see section \ref{TODO}). However, reading from CSV files isn't an option latency wise, therefore, the PerformanceMap class loads this data into an optimized in-memory structure at startup. 
...
The query function is the brain of this component, it takes the matrix dimensions, the data type, and a boolean flag \texttt{add\_jit}, part of the code is shown below:
...
All of this tweaks exhibits, as it will be shown in the next section, a good behavior in predicting the time of completition of a task, furthermore the code used for the black box analysis is intirely in the tests.hpp file, found in Appendix \ref{TODO}.
...
Below is a simplified view of the class structure and its key components:

\begin{lstlisting}[caption={Profiler Class Structure}, language=C++] 
...
A simple example, a batch of 10 tasks distributed between two accelerators A and B, where the logic assigns: 4.7 tasks to A and 5.3 tasks to B. If we simply truncate integers A gets 4 tasks and B gets 5 tasks, so the total assigned will be 9 and we are missing 1 task.
