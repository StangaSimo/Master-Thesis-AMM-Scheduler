...
\begin{lstlisting}[caption={Split Logic: Sub Task dispatch}, language=C++] 
/* .... */
...
The core relies on the \texttt{bt\_ms} array, wich tracks the load assigned to each accellerator so far (in the current batch). The load stand for the accumulated ms expected for that particular accellerator. 
